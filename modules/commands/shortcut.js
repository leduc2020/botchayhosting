                  ...
                  if (e.input_type === 'autosend') {
                      if (e.hours === now) {
                          ...
                          if (e.uri) try {
                              ...
                              else if (e.uri === 'vdgai') msg.attachment = global.ytb_rst.splice(0, 1);
                              else if (e.uri === 'vdanime') msg.attachment = global.vdanime.splice(0, 1);
                          } catch { };
  ...
  switch (logMessageType) {
      case 'log:subscribe': {
          ...
          try {
              ...
              else if (join.uri === 'vdgai') msg.attachment = global.ytb_rst.splice(0, 1)
                              else if (join.uri === 'vdanime') msg.attachment = global.vdanime.splice(0, 1)
          } catch { };
      ...
      case 'log:unsubscribe': {
          ...
          try {
              ...
              if (/^https:\/\//.test(leave.uri)) msg.attachment = await stream_url(leave.uri);
               else if (leave.uri === 'vdgai') msg.attachment = global.ytb_rst.splice(0, 1)
                              else if (leave.uri === 'vdanime') msg.attachment = global.vdanime.splice(0, 1)
...
module.exports.handleEvent = async function ({ event, api, Users }) {
  ...
  if ((dataThread = mentions.length > 0 ? data.find(item => typeof item.tag_id == 'string' && mentions.includes(item.tag_id)) : false) || (dataThread = data.find(item => (item.input || '').toLowerCase() == body.toLowerCase()))) {
      ...
      if (dataThread.uri === 'vdanime') msg.attachment = global.vdanime.splice(0, 1);
        else if (dataThread.uri === 'vdgai') msg.attachment = global.ytb_rst.splice(0, 1);
  ...
  try {
      ...
      switch (handleReply.type) {
          ...
          case "requireOutput": {
              if (body.length == 0) return api.sendMessage("❎ Câu trả lời không được để trống", threadID, messageID);
              api.unsendMessage(handleReply.messageID);
              return api.sendMessage(`📌 Reply tin nhắn này bằng tệp video/ảnh/mp3/gif hoặc nếu không cần bạn có thể reply tin nhắn này và nhập 's' hoặc muốn random video theo data api có sẵn thì nhập 'vdgai' hoạc 'vdanime' nhóe`, threadID, function (error, info) {
              return global.client.handleReply.push({
                  type: "requireGif",
                  name,
                  author: senderID,
      ...
      case "requireGif": {
        let uri = body;
        if (!['s','vdgai','vdanime'].includes(body)){
    ...
    case 'autosend.input_time': {
        ...
        api.sendMessage(`📌 Reply tin nhắn này bằng tệp video / ảnh / mp3 / gif hoặc nếu không cần bạn có thể reply tin nhắn này và nhập 's' hoặc muốn random video theo data api có sẵn thì nhập 'vdgai' hoạc 'vdanime'`, threadID, (err, data)=>{
            global.client.handleReply.push({
                ...data,
                author: senderID,
                name: exports.config.name,
          ...
          if (data.length == 0) return api.sendMessage("❎ hiện tại nhóm của bạn chưa có shortcut nào được set", threadID, messageID);
              ...
              for (const single of data) {
                  const inputDisplay = ({
                      ...
                      join: _ => `join noti`,
                      leave: _ => `leave noti`
                  }[single.input_type] || (_ => single.input || 'unknown'))();
                  const mediaStatus = single.uri !== 's' ? 'Có' : 'Không';
                  array.push(`${n++}.${inputDisplay} => ${single.output}\n📂 File media: ${mediaStatus}`);
              }
              return api.sendMessage(`📝 Danh sách các shortcut:\n${array.join('\n')}\n\n(Reply theo số để xóa shortcut)`, threadID, function (error, info) {
                  global.client.handleReply.push({
                      type: "delShortcut",
                      name,
                      author: senderID,
