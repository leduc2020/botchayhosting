...
<<<<<<< HEAD
    ...
    description: "Ảnh toilet của đứa tag",
...
module.exports.run = async ({ event, api, args, Users }) => {
    try {
        ...
        const background = await Canvas.loadImage('https://hita.com.vn/tinymce/uploads/images/fix-content/bon-cau-bi-tran-nuoc.gif');
        ...
        // Căn giữa avatar (205x205) trong canvas 960x634
        const x = (canvas.width - 205) / 2;  // Căn giữa theo chiều ngang
        const y = (canvas.height - 205) / 2; // Căn giữa theo chiều dọc
        ctx.drawImage(await Canvas.loadImage(avatar), x, y, 205, 205);
        const imageBuffer = canvas.toBuffer();
        fs.writeFileSync(path_toilet, imageBuffer);
        api.sendMessage({attachment: fs.createReadStream(path_toilet, {'highWaterMark': 128 * 1024}), body: "sao lại chui vô đó"}, event.threadID, () => fs.unlinkSync(path_toilet), event.messageID);
    }
    catch(e) {
