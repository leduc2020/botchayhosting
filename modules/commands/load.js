...
<<<<<<< HEAD
const loadCommand = function ({ moduleList, threadID, messageID, isAll }) {
  ...
  if (errorList.length != 0) api.sendMessage('⚠️ Những lệnh vừa xảy ra sự cố khi hệ thống loading: ' + errorList.join('\n'), threadID, messageID);
  if (isAll) {
    const successCount = moduleList.length - errorList.length;
    api.sendMessage(`Đã load thành công ${successCount} lệnh ✅`, threadID, messageID);
  } else {
...
module.exports.run = async function ({ api, event, args }) {
  ...
  if (args.length === 0) return api.setMessageReaction("⚠️", event.messageID, (err) => {}, true);
  
  var moduleList = [];
  const isAll = args[0].toLowerCase() === "all";
  if (isAll) {
    const commandFiles = fs.readdirSync(__dirname).filter(file => file.endsWith('.js'));
    moduleList = commandFiles.map(file => file.replace('.js', ''));
  } else {
    moduleList = args;
  }
  
  if (moduleList.length === 0) return api.setMessageReaction("⚠️", event.messageID, (err) => {}, true);
  else return loadCommand({ moduleList, threadID, messageID, isAll });
};
=======
